class JunOS < Oxidized::Model
  ...
  def sanitize_encoding(cfg)
    cfg.force_encoding('UTF-8').scrub('')
  end

  cmd :all do |cfg|
    cfg = cfg.cut_both if screenscrape
    cfg = sanitize_encoding(cfg)
  ...
  post do
    ...
    case @model
    when 'mx960'
      out << cmd('show chassis fabric reachability') { |cfg| comment sanitize_encoding(cfg) }
    when /^(ex22|ex3[34]|ex4|ex8|qfx)/
      out << cmd('show virtual-chassis') { |cfg| comment sanitize_encoding(cfg) }
    end
    out
  end

  cmd('show chassis hardware') { |cfg| comment sanitize_encoding(cfg) }
  ...
  cmd('show system license keys') { |cfg| comment sanitize_encoding(cfg) }
